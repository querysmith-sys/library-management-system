...
router.post("/add-clerk", async (req, res, next) => {
    ...
    const  queryResult = await pool.query(`INSERT INTO clerks (clerkname, clerkemail) VALUES($1, $2)`,[sanitizedclerkname,sanitizedclerkemail]);
    if (queryResult.rowCount == 0) {
      return res
        .status(500)
        .json({ success: false, error: "query was not accepted" });
    }
   return res
      .status(200)
...
router.get("/clerks", async (req, res, next) => {
  try {
    const result = await pool.query(`SELECT * FROM clerks WHERE deleted_at IS NULL`);
...
router.patch("/edit-clerk", async (req, res, next) => {
    try {
    ...
    if (clerkname === undefined && clerkemail === undefined) {
     ...
     await pool.query(
      `UPDATE clerks SET clerkname = COALESCE($1, clerkname), clerkemail = COALESCE($2, clerkemail) WHERE clerk_id = $3`,
...
router.delete("/archieve-clerk", async (req, res, next) => {
  try {
    ...
    await pool.query(`UPDATE clerks SET deleted_at = NOW() WHERE clerk_id = $1`, [clerk_id]);
...
router.get('/admin-stats', async (req, res, next) => {
  try {
    const result = await pool.query(`SELECT 
      ...
      (SELECT COUNT(*) FROM clerks WHERE deleted_at IS NULL) AS total_clerks,
