...
router.post('/add-book', async(req,res,next) => {
    try {
        ...
        if(title == null || author == null || isbn == null || total_copies == null || available_copies == null) {
            return res.status(400).json({success:false,error:"missing input"});
        }
        if(typeof title !== "string" || typeof author !== "string" || !Number.isInteger(isbn) || !Number.isInteger(total_copies) || !Number.isInteger(available_copies)) {
        ...
        if(sanitizedtitle.length == 0 || sanitizedauthor.length == 0) {
            return res.status(400).json({success:false,error:"title or author is empty."});
        }
...
router.get('/generate-fine-report', async(req,res) => {
     try {
        ...
        if(typeof fine_amt !== "number" || !Number.isInteger(fine_amt)) {
            return res.status(400).json({success:false,error:"invalid fine_amt"});
        }
        if(typeof member_id !== "number" || !Number.isInteger(member_id)) {
            return res.status(400).json({success:false,error:"invalid member_id"});
        }
...
router.post('/members', async(req,res,next) => {
    try {
        ...
        if(membername == null || memberemail == null) {
            return res.status(400).json({success:false,error:"membername or memberemail is missing."});
        }
        if(typeof membername !== "string" || typeof memberemail !== "string") {
            return res.status(400).json({success:false,error:"invalid membername or memberemail."});
        }
        ...
        if(sanitizedmembername.length == 0 || sanitizedmemberemail.length == 0) {
            return res.status(400).json({success:false,error:"membername or memberemail is empty."});
        }
...
router.get('/members' , async(req, res, next) => {
    try {
        ...
        if(result.rowCount === 0) {
           return res.status(404).json({success:false, error:"no member data found"});
        }
...
router.patch('/members' , async(req, res, next) => {
    try {
        ...
        if(typeof member_id !== "number" || !Number.isInteger(member_id)) {
            return res.status(400).json({success:false, message:"provide an integer input"});
        } 
        let sanitizedmembername;
        let sanitizedmemberemail;
        if(membername !== undefined ) {
            if(membername === null) {
                return res.status(400).json({success:false, message:"membername cannot be null"});
            } 
            if(typeof membername !== "string") {
                 return res.status(400).json({success:false, message:"provide valid membername"});
            }
               sanitizedmembername = membername.trim();
            if(sanitizedmembername.length === 0) {
                return res.status(400).json({success:false,error:"membername is empty"});
            }   
        }
        if(memberemail !== undefined ) {
            if(memberemail === null) {
                return res.status(400).json({success:false, message:"memberemail cannot be null"});
            } 
            if(typeof memberemail !== "string") {
                 return res.status(400).json({success:false, message:"provide valid memberemail"});
            }
              sanitizedmemberemail = memberemail.trim();
            if(sanitizedmemberemail.length === 0) {
                return res.status(400).json({success:false,error:"memberemail is empty"});
            }   
        }
       
        if(membername === undefined && memberemail === undefined) {
            return res.status(400).json({success:false, message:"provide membername or memberemail to update"});
        }

        await pool.query(`UPDATE members SET membername = COALESCE($1, membername), memberemail = COALESCE($2, memberemail) WHERE member_id = $3`,[sanitizedmembername, sanitizedmemberemail, member_id]);  
        return res.status(200).json({success:true, message:"member info updated successfully"});
    } catch (error) {
        next(error);
    }
})
...
router.delete('/members', async(req, res, next) => {
    try {
        ...
        await pool.query(`UPDATE members SET deleted_at = NOW() WHERE member_id = $1`,[member_id]);
    } catch (error) {
        next(error);
    }
})
