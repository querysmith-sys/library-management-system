...
const TokenVerification = async (req, res, next) => {
    ...
    try {
        ...
        if (error.name === 'TokenExpiredError') {
            ...
            const queryResult = await pool.query(`SELECT * FROM refresh_tokens WHERE user_id = {$1}`, [decoded_refreshToken.payload.user_id]);
            ...
            res.status(200).json({success:true, accessToken:new_access_Token});
